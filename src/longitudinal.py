...
class YController():
    ...
    def __init__(self):
        ...
        self.initiate = rospy.Subscriber("/bluerov/lon_setpoint", Setpoint, self.setpoint_callback)
        self.thrust_pub = rospy.Publisher("thrust", Float64, queue_size=1)
        self.stable = rospy.Publisher("lon_stable", Bool, queue_size=1)
    ...
    def get_thrust(self):
            ...
            #        vmin = -0.2
                ...
                if thrust > vmax:
                    ...
                    thrust = vmin
                print("lon_error", error)
                self.publish(thrust)
                if abs(error) < 0.05:
                    self.j = self.j +1
                    if self.j >30:
                        self.lon_stable = True
                        self.stable.publish(self.lon_stable)
                        self.j = 0
                else:
                    self.j = 0
                    self.lon_stable = False
                    self.stable.publish(self.lon_stable)
